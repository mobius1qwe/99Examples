...
begin
    try
        json := dm.RequestCategoria.Response.JSONValue.ToString;
        jsonArray := TJSONObject.ParseJSONValue(TEncoding.UTF8.GetBytes(json), 0) as TJSONArray;

        // Se deu erro...
        if dm.RequestCategoria.Response.StatusCode <> 200 then
        begin
            showmessage(retorno);
            exit;
        end;
...
begin
    try
        json := dm.RequestGrupo.Response.JSONValue.ToString;
        jsonArray := TJSONObject.ParseJSONValue(TEncoding.UTF8.GetBytes(json), 0) as TJSONArray;

        // Se deu erro...
        if dm.RequestGrupo.Response.StatusCode <> 200 then
        begin
            showmessage(retorno);
            exit;
        end;
...
begin
    ...
    dm.RequestCategoria.Params.Clear;
    dm.RequestCategoria.ExecuteAsync(ProcessarCategoria, true, true, ProcessarCategoriaErro);
end;

procedure TFrmCategoria.ListarGrupo(cat: string);
begin
    // Buscar categorias no servidor...
    dm.RequestGrupo.Params.Clear;
    dm.RequestGrupo.AddParameter('categoria', categoria);
    dm.RequestGrupo.ExecuteAsync(ProcessarGrupo, true, true, ProcessarCategoriaErro);
end;
