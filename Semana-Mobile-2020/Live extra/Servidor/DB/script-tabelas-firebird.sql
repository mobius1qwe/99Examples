...
CREATE TABLE TAB_PRODUTO_OPCIONAL (
    ID_OPCAO INT NOT NULL PRIMARY KEY,
    ID_PRODUTO INT,
    DESCRICAO    VARCHAR(100),
    VALOR       DECIMAL(12,2)
);

create generator gen_opcional_id;

CREATE TRIGGER TR_OPCIONAL FOR TAB_PRODUTO_OPCIONAL
ACTIVE BEFORE INSERT POSITION 0
AS
BEGIN
    new.ID_OPCAO =gen_id(gen_opcional_id, 1);
END


ALTER TABLE TAB_COMANDA_CONSUMO ADD OBS_OPCIONAL VARCHAR(500);

ALTER TABLE TAB_COMANDA_CONSUMO ADD VALOR_OPCIONAL DECIMAL(12,2);
