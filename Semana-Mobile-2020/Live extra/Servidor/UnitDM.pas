...
type
  Tdm = class(TServerMethodDataModule)
    ...
    procedure DWEventsEventsListarOpcionalReplyEvent(var Params: TDWParams;
      var Result: string);
    procedure DWEventsEventsTransferirComandaReplyEvent(var Params: TDWParams;
      var Result: string);
  private
...
procedure Tdm.DWEventsEventsTransferirComandaReplyEvent(var Params: TDWParams;
  var Result: string);
var
    json : TJsonObject;
    qry : TFDQuery;
begin
    try
        ...
        try
            // Marca a comanda "PARA" como aberta...
            qry.Active := false;
            qry.SQL.Clear;
            qry.SQL.Add('UPDATE TAB_COMANDA SET STATUS = ''A'', ');
            qry.SQL.Add('DT_ABERTURA = COALESCE(DT_ABERTURA, current_timestamp) ');
            qry.SQL.Add('WHERE ID_COMANDA = :ID_COMANDA_PARA');
            qry.ParamByName('ID_COMANDA_PARA').Value := Params.ItemsString['id_comanda_para'].AsString;
            qry.ExecSQL;

            // Transfere o consumo da comanda "DE" para a comanda "PARA" ...
            qry.Active := false;
            qry.SQL.Clear;
            qry.SQL.Add('UPDATE TAB_COMANDA_CONSUMO SET ID_COMANDA=:ID_COMANDA_PARA ');
...
begin
    try
        ...
        try
            ...
            qry.Active := false;
            qry.SQL.Clear;
            qry.SQL.Add('INSERT INTO TAB_COMANDA_CONSUMO(ID_COMANDA, ID_PRODUTO, ');
            qry.SQL.Add('QTD, VALOR_TOTAL, OBS_OPCIONAL, VALOR_OPCIONAL, OBS)');
            qry.SQL.Add('VALUES(:ID_COMANDA, :ID_PRODUTO, :QTD, :VALOR_TOTAL, ');
            qry.SQL.Add(':OBS_OPCIONAL, :VALOR_OPCIONAL, :OBS)');
            qry.ParamByName('ID_COMANDA').Value := Params.ItemsString['id_comanda'].AsString;
            qry.ParamByName('ID_PRODUTO').Value := Params.ItemsString['id_produto'].AsInteger;
            ...
            qry.ParamByName('VALOR_TOTAL').Value := (Params.ItemsString['vl_total'].AsFloat / 100) +
                                                    (Params.ItemsString['vl_opcional'].AsFloat / 100);
...
begin
    try
        ...
        qry.Active := false;
        qry.SQL.Clear;
        qry.SQL.Add('SELECT C.*, P.DESCRICAO');
