...
function Tdm.TransferirComanda(id_comanda_de, id_comanda_para: string;
                               out erro: string): boolean;
var
    json : string;
    jsonObj: TJsonObject;
begin
    ...
    with RequestTransferir do
    begin
        Params.Clear;
        AddParameter('id_comanda_de', id_comanda_de, TRESTRequestParameterKind.pkGETorPOST);
        AddParameter('id_comanda_para', id_comanda_para, TRESTRequestParameterKind.pkGETorPOST);
        Execute;
    end;

    if RequestTransferir.Response.StatusCode <> 200 then
    begin
        Result := false;
